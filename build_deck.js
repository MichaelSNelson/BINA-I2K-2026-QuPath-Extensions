...
// side-by-side figure: two panels of equal size, each labeled, plus a caption.
...
function tileStep(title, shown, exploded, caption, shade) {
  ...
  if (shade && !exploded) {
    // the strip the neighboring tiles share
    s.addShape(pptx.ShapeType.rect, {
...

{
  ...
  s.addText([
    ...
    { text: 'Kristin Gallik, whose concept and scripts the Confusion Matrix grew from.  ·  Pete Bankhead and the QuPath team.  ·  CT-FIRE, CurveAlign, TACS and TWOMBLI for the fiber work.  ·  CytoMAP and QuBaLab for bringing clustering into QuPath.  ·  QUAREP-LiMi for the reporting standards.  ·  The image.sc community, where several of these features were first requested.\n', options: { fontSize: 16, color: INK, paraSpaceAfter: 9 } },
...
content('What this hour covers', [
  ...
  'Three are demonstrated but not practiced \u2014 I will say why each time; two more were Sara McArdle\u2019s session yesterday',
...

{
  ...
  s.addText([
    ...
    { text: 'Already fixed: QPSC, the Confusion Matrix and the fiber tools are shown regardless — and Channel Names Viewer and Classify Object Subset were covered in Sara McArdle’s session yesterday.\n', options: { fontSize: 18, italic: true, color: MUT, paraSpaceAfter: 12 } },
...
content('Dialog Manager, Channel Names, Classify Subset', [
  ...
  'Channel Names Viewer — a floating, color-coded legend of the selected channels, so you are not re-reading the brightness dialog to find out which is which',
...
content('The two wands', [
  ...
  { t: 'Four color-space modes: grayscale, RGB, subtle stain differences, or selecting by hue' },
  ...
  { t: 'Push a traced boundary outward, erase backwards from an endpoint, or cut one polyline in two — both halves keep class, name and color' },
...
demo('Both wands, live', '▶  LIVE  ·  10 MIN',
  [
    ...
    'Scissors mode — cut one polyline in two, both halves keeping class, name and color',
...
content('Project Metadata Browser', [
  ...
  'Rename or remove a key across every image in one operation',
     ...
     note: 'Sorting by a recognized column makes the bad reads stand out as outliers.', repos: [{ name: 'Metadata Browser', slug: 'uw-loci/qupath-extension-project-metadata-browser' }] });
...
tileStep('Four captures, one field of view', 4, true,
  'Each tile is a separate exposure. The stage moved between them, and every neighboring pair was told to share about 10% of its width so there is common tissue to match on.', false);
...
tileStep('Where the stage said they were', 4, false,
  'All four, placed on the recorded coordinates alone. Every neighboring pair now shares a strip.', true);
...
content('Tiles to Pyramid — how the mosaic is assembled', [
  ...
  { t: 'A spanning tree would keep 99 of the 180 neighbor edges on a 10×10 grid and throw away 81, so nothing is ever asked to close a loop' },
...
content('Deep learning pixel classification', [
  ...
  'Brightfield and multi-channel fluorescence, with channel selection and intensity normalization',
  'Train across several project images at once for representative sampling',
], { kicker: 'For when the built-in classifier is not enough: subtle textures, classes that differ by architecture rather than color.', repos: [{ name: 'DL Pixel Classifier', slug: 'uw-loci/qupath-extension-dl-pixel-classifier' }] });
...
content('QP-CAT — multiplexed cell analysis', [
  ...
  'Label a small subset by hand and have the rest of the project labeled for you — no object classifier involved',
...
content('Why fiber architecture', [
  ...
  'In breast pathology, straightened fibers running perpendicular to the tumor boundary act as tracks for invading cells',
...
twoCol('Two complementary tools',
  ...
  [
    ...
    'Straightness: wavy versus straightened fibers, per window and per ROI',
    'Morphometrics: coverage, length, branching, fractal dimension, gaps',
    'Texture: the information fiber-tracing misses entirely',
  ],
  'TME-Quant — trace the fibers',
  [
    'Individual fibers extracted and committed back as objects',
    'Set the threshold by eye, with the mask shown before you commit',
    'Trace a few real fibers yourself, and it tunes its own parameters to match',
    'Fibers classified by orientation relative to a tumor boundary',
  ],
...
content('Why this changes the rest', [
  'The region you analyzed is the region you acquire at high resolution',
...
twoCol('What worked, and what did not',
  ...
  [
    ...
    'GUI behavior — nothing catches a dialog opening off-screen except a human',
...
content('What is underneath', [
  ...
  'Normalization statistics sampled across the image, or reused from training, so every tile is scaled the same way',
...
content('What we are not covering', [
  ...
  'Unsupervised clustering · rule-based marker gating · propagation from a small hand-labeled subset · applying an existing classifier to a chosen subset instead of everything',
