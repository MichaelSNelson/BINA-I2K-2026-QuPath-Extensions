...
> This tool is demonstrated rather than practised, but the walkthrough below is complete, so you can follow it yourself once you have access to the tool.
...
Frangi / Sato / Meijering ridge filter to enhance line-like structures) or supplied, and it will
...
**Morphometrics** are shape statistics of the segmented fibre graph: HDM coverage, total fibre
...
**Texture** is the information fibre-tracing misses. A dense uniform mat and a sparse clumped
...
2. The documented setup path is **Windows-only** (MSYS2 UCRT64). First run installs several
   hundred MB and builds the FIRE engine, taking 5–15 minutes.
