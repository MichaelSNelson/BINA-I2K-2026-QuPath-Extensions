...
The OCR engine is built into the extension. What it does not include is its language data:

`Extensions > OCR for Labels > OCR Settings...` → **Required Downloads** has a link for each,
...
- [**osd.traineddata**](https://github.com/tesseract-ocr/tessdata_fast/raw/main/osd.traineddata) (11 MB) — handles rotated labels. The dialog lists it as optional; the exercise uses it, so get both.
...
| **Track B** (QP-CAT, Cluster 3D Navigator) | **`multiplex-synthetic-data-demo-project-clustered.zip`**, [direct link](https://github.com/uw-loci/multiplex-synthetic-data/releases/download/v1.2/multiplex-synthetic-data-demo-project-clustered.zip). The demo project with a saved KMeans k=6 clustering already in it, so you can read the results without waiting for a run | **23 MB** |
...
| **Track D** (Tiles to Pyramid) | **`Tiles-to-pyramid.zip`**, [direct link](https://github.com/MichaelSNelson/BINA-I2K-2026-QuPath-Extensions/releases/download/data-v1/Tiles-to-pyramid.zip). Four folders of raw acquisition tiles — a MicroManager multi-channel set, a QPSC multi-channel set, and two polarization angles. Unzip it anywhere; you point the extension at a folder inside it rather than opening a project | **459 MB** |
